...
- **Timelock ordering.** In a two-chain swap the leg released first by the preimage must have the shorter expiry, giving the party who learns it on-ledger enough time to claim on the other chain. The reference implementation documents the required margins.
- **Blocked receivers.** Enactment creates a receiver holding, so registry transfer rules apply (section 3.6); a paused or blocked account fails the rule, returning funds to the authorizer at expiry, losing the Canton leg of a two-chain swap while the external leg stays claimable. Counterparties SHOULD confirm receiver status before funding the external leg, and registries SHOULD surface it via the choice-context endpoint (section 3.8).
- **Submission delay.** Registries that delay between preparation and execution (Canton Coin allows up to 24 hours under CIP-0107) shrink the effective enactment window. Wallets MUST account for that delay when choosing `expiresAt` and `Guard_Before`, and registries SHOULD publish `min-duration` accordingly.
- **Authoring errors.** A rule whose `fixedLegs` exceed the remaining amount after earlier partial consumption cannot fire. Wallets SHOULD validate that every sequence of firings remains enactable, and registries MAY reject terms whose legs exceed `amount` at creation.
- **Enactor discretion.** `Outcome_Release` with non-empty `receivers` gives the enactor amount discretion, bounded to the receiver set and to the remaining amount net of `fixedLegs`; authorizers SHOULD pair it with `Guard_Parties` naming the decision-maker. Supplied legs MAY be empty even when both fields are non-empty, letting the enactor release `fixedLegs` alone and leave the remainder locked, so authorizers SHOULD size a fixed fee to the decision-maker's obligation and rely on `expiresAt` as backstop.
- **Preimage evaluation.** A preimage in `witness.preimages` is hashed and compared under the lock signatories' authority (section 3.3) at `Enact`. The enactor supplies the witness but is not trusted to evaluate it: a lying enactor cannot make a guard appear satisfied.
- **Amendment.** `Amend` is unanimous among the authorizer and every named party, so no position changes without consent, and it never returns `Pending`. A fired rule cannot be reinstated by amendment: registries retain `enactedRuleIds` for the lifetime of a `lockId`.
- **Expired locks and resources.** An expired lock can only be `Expire`d or `Cancel`led, never enacted or amended; registries MAY exercise `Expire` with admin authority for cleanup, since funds only return to the authorizer, and MAY let the admin alone exercise `ConditionalLockInstruction_Withdraw` at or after `terms.expiresAt` for abandoned instructions (section 3.2). Registries whose holdings accrue a fee MUST provide an admin cleanup path, and MUST NOT fire any rule after expiry.
- **Preimage disclosure.** A revealed preimage is visible to the enactment transaction's stakeholders and to anyone they disclose it to; `conditionalLock_enactExtraObservers` bounds that visibility as stated in section 3.9. A digest reused across locks is a wallet error, not a protocol weakness, but wallets SHOULD warn.
